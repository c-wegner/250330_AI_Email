...
// Function to format file_as field based on entity type
function formatFileAs(name: string, entity: string): string {
    if (entity === 'Company') {
        // For companies, file_as is the same as the name
        return name;
    } else {
        // For individuals, format as last name, first name
        const nameParts = name.trim().split(' ');
        if (nameParts.length === 1) {
            return name; // Only one name, return as is
        } else if (nameParts.length === 2) {
            // Simple first and last name
            const [firstName, lastName] = nameParts;
            return `${lastName}, ${firstName}`;
        } else {
            // Handle multiple parts - assume last part is last name
...
// Initialize the database with client data
export function initializeClientDatabase() {
    const clientDB = new ClientDatabase();
    
    // Client data from the provided CSV
    const clientData = [
    ...
    const existingClients = clientDB.getClientBook().clients;
    if (existingClients.length > 0) {
        console.log("Client database already initialized with", existingClients.length, "clients");
        return;
    }

    // Add each client to the database
    for (const client of clientData) {
        const fileAs = formatFileAs(client.name, client.entity);
        
        // Create email object
        const emailObj: IEmail = {
            address: client.email,
            name: client.name,
            role: client.entity === 'Company' ? 'Main Contact' : 'Client',
            principal: true,
            primary: true
        };
        
        // Create new client
        const newClient = clientDB.addClient({
            name: client.name,
            file_as: fileAs,
            status: 'active',
            emails: [emailObj],
            description: `${client.entity} client`,
            referal_source: ''
        });
    }
    
    console.log("Initialized client database with", clientData.length, "clients");
}
